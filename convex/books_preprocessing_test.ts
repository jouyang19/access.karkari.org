  ...
  handler: async (ctx): Promise<{ success: boolean; message: string }> => {
    ...
    for (const doc of documents) {
      try {
        ...
        await ctx.runMutation(api.books_test.create, processedData);
      } catch (error) {
...
For chapter: v.object({ number: v.float64(), sectionTitle: v.string(), title: v.string(), }), To determine if isChapterStart is true or false, match the page number to the exact page number of that chapter in the table of contents.

To determine if isSectionStart is true or false, match the page number to the exact page number listed in the table of contents. Sections are listed under each chapter. Note that all chapters are also sections.

Also infer the sectionTitle by seeing if it is either an exact page number or between two page numbers on the table of contents.
